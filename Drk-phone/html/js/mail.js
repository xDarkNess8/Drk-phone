...
$(document).on('click', '#accept-mail', function(e) {
    ...
    $.post('https://Drk-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e) {
    e.preventDefault();
    var MailData = $("#" + OpenedMail).data('MailData');
    $.post('https://Drk-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0 + "vh"
...
NN.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(NN.Phone.Data.PlayerData.charinfo.firstname + "." + NN.Phone.Data.PlayerData.charinfo.lastname + "@sdshop.com");
...
$(document).on('click', '#new-advert-submit', function(e) {
    ...
    if (Advert !== "") {
        ...
        $.post('https://Drk-phone/PostAdvert', JSON.stringify({
            message: Advert,
        }));
    } else {
