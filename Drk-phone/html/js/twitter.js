...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('https://Drk-phone/ClearMentions');
        }
...
$(document).on('click', '.twitter-new-tweet', function(e){
    ...
    NN.Phone.Animations.TopSlideDown(".twitter-new-tweet-tab", 450, 0);
});

NN.Phone.Notifications.LoadTweets = function(Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            var TwtMessage = NN.Phone.Functions.FormatTwitterMessage(clean);
            var today = new Date();
...
NN.Phone.Notifications.LoadMentionedTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets.length > 0) {
...
NN.Phone.Functions.FormatTwitterMessage = function(TwitterMessage) {
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        $.post('https://Drk-phone/PostNewTweet', JSON.stringify({
            Message: TweetMessage,
            Date: CurrentDate,
            Picture: NN.Phone.Data.MetaData.profilepicture
        }), function(Tweets){
            NN.Phone.Notifications.LoadTweets(Tweets);
        });
        ...
        NN.Phone.Animations.TopSlideUp(".twitter-new-tweet-tab", 450, -120);
    } else {
        NN.Phone.Notifications.Add("fab fa-twitter", "Twitter", "Fill a message!", "#1DA1F2");
    }
});
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('https://Drk-phone/GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
            NN.Phone.Notifications.LoadHashtagMessages(HashtagData.messages);
        });
...
function CopyMentionTag(elem) {
    ...
    NN.Phone.Notifications.Add("fab fa-twitter", "Twitter", $(elem).data('mentiontag')+ " copied!", "rgb(27, 149, 224)", 1250);
    document.execCommand("copy");
    $temp.remove();
}

NN.Phone.Notifications.LoadHashtags = function(hashtags) {
    if (hashtags !== null) {
...
NN.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
...
$(document).on('click', '.twitter-hashtag', function(event){
    ...
    NN.Phone.Notifications.LoadHashtagMessages(TweetData.messages);
