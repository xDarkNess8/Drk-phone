...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    NN.Phone.Functions.SetupChatMessages(ChatData);

    $.post('https://Drk-phone/ClearAlerts', JSON.stringify({
...
NN.Phone.Functions.GetLastMessage = function(messages) {
...
NN.Phone.Functions.LoadWhatsappChats = function(chats) {
    $(".whatsapp-chats").html("");
    $.each(chats, function(i, chat){
        ...
        var LastMessage = NN.Phone.Functions.GetLastMessage(chat.messages);
...
NN.Phone.Functions.ReloadWhatsappAlerts = function(chats) {
    $.each(chats, function(i, chat){
        if (chat.Unread > 0 && chat.Unread !== undefined && chat.Unread !== null) {
            $(".unread-chat-id-"+i).html(chat.Unread);
            $(".unread-chat-id-"+i).css({"display":"block"});
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        $.post('https://Drk-phone/SendMessage', JSON.stringify({
        ...
        NN.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "You can't send a empty message!", "#25D366", 1750);
    }
});
...
// NN.Phone.Functions.SetupChatMessages = function(cData, NewChatData) {
//     if (cData) {
//         OpenedChatData.number = cData.number;

//         if (OpenedChatPicture == null) {
//             $.post('https://Drk-phone/GetProfilePicture', JSON.stringify({
...
//                 var Sender = "me";
//                 if (message.sender !== NN.Phone.Data.PlayerData.citizenid) { Sender = "other"; }
//                 var MessageElement
...
$(document).on('click', '.whatsapp-shared-location', function(e){
    ...
    $.post('https://Drk-phone/SharedLocation', JSON.stringify({
